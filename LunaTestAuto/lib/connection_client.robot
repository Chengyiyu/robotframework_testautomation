...
Library           SSHLibrary    timeout=${SSH_Connection_Timeout}
...
Open Connection And Log In
    [Arguments]    ${username}=${OPENBMC_USERNAME}    ${password}=${OPENBMC_PASSWORD}    &{connection_args}
    ...
    ${has_host}=    Run Keyword and Return Status    Dictionary Should Contain Key    ${connection_args}    host
    Run Keyword If    ${has_host} == ${FALSE}    Set To Dictionary    ${connection_args}    host=${OPENBMC_HOST}
    ...
    ${has_port}=    Run Keyword and Return Status    Dictionary Should Contain Key    ${connection_args}    port
    ...
    Run Keyword If    '${SSH_PORT}' != '${EMPTY}' and ${has_port} == ${FALSE}    Run Keywords    Set To Dictionary    ${connection_args}    port=${SSH_PORT}
    ...    AND    SSHLibrary.Open connection    &{connection_args}
...
Open Connection for SCP
    ...
    Run Keyword If    '${SSH_PORT}' == '${EMPTY}'    scp.Open connection    ${OPENBMC_HOST}    username=${OPENBMC_USERNAME}    password=${OPENBMC_PASSWORD}
    ...    ELSE    Run Keyword    scp.Open connection    ${OPENBMC_HOST}    port=${SSH_PORT}    username=${OPENBMC_USERNAME}    password=${OPENBMC_PASSWORD}
...
Validate Or Open Connection
    ...
    Run Keyword If    '${host}' == '${EMPTY}'    Should Not Be Equal    ${alias}    None    msg=Need to provide a host or an alias.    values=False
    # Search the dictionary to see if it includes the host and alias.
    ${host_exists}=    Run Keyword and Return Status    Dictionary Should Contain Key    ${connection_args}    host
    ${alias_exists}=    Run Keyword and Return Status    Dictionary Should Contain Key    ${connection_args}    alias
    # Add the alias and host back into the dictionary of connection arguments,
    # if needed.
    Run Keyword If    '${host}' != '${EMPTY}' and ${host_exists} == ${FALSE}    Set to Dictionary    ${connection_args}    host    ${host}
    Run Keyword If    '${alias}' != 'None' and ${alias_exists} == ${FALSE}    Set to Dictionary    ${connection_args}    alias    ${alias}
    @{open_connections}=    Get Connections
    # If there are no open connections, open one and return.
    Run Keyword If    '${open_connections}' == '[]'    Open Connection and Log In    &{connection_args}
    ...
    FOR    ${connection}    IN    @{open_connections}
        ...
        Run Keyword If    ${given_alias} and ${given_host} and ${alias_match} and ${host_match}    Run Keywords    Switch Connection    ${alias}
        ...    AND    Log to Console    Found connection. Switched to ${alias} ${host}
        ...    AND    Return From Keyword If    ${alias_match} and ${host_match}
        ...    ELSE    Run Keyword If    ${given_alias} and ${no_host} and ${alias_match}    Run Keywords    Switch Connection    ${alias}
        ...    AND    Log to Console    Found connection. Switched to: ${alias}
        ...    AND    Return From Keyword If    ${alias_match}
        ...    ELSE    Run Keyword If    ${given_host} and ${no_alias} and ${host_match}    Run Keywords    Switch Connection    ${connection.index}
        ...    AND    Log to Console    Found Connection. Switched to: ${host}
        ...    AND    Return From Keyword If    ${host_match}
    END
